...
def index():
    ...
    for stock in stock_symbols:
        ...
        stock_information['price'] = stock_info['price']
        stock_information['shares'] = sum_shares[0]['SUM(Num_shares)']
        stock_information['total'] = stock_information['shares'] * stock_information['price']
    ...
    if stock_symbols == []:
        ...
        for i in range(len(portfolio)):
            sum_total += portfolio[i]['total']
        sum_total += cash[0]['cash']



        return render_template("index.html", portfolio = portfolio, cash = cash[0]['cash'], sum_total=sum_total)
...
def buy():
    ...
    if request.method == "POST":
        ...
        if int(row[0]["cash"]) > total_price:
            ...
            db.execute("UPDATE users SET cash = cash - :price WHERE id = :user_id", price=usd(total_price), user_id=session["user_id"])# if user has enough cash
