...
def index():
    ...
    stock_symbols = db.execute("SELECT symbol FROM transactions WHERE user_ID = :user_id GROUP BY symbol",
    ...
    if stock_symbols == []:
        ...
        return render_template("index.html", portfolio=portfolio, cash=usd(cash[0]['cash']), sum_total=usd(sum_total))
...
def buy():
    ...
    if request.method == "POST":
        ...
        if int(row[0]["cash"]) < total_price:
            return apology("Not enough cash, sorry", 400)
        ...
        if int(row[0]["cash"]) > total_price:
            ...
            db.execute("UPDATE users SET cash = cash - :price WHERE id = :user_id",
                        price=total_price, user_id=session["user_id"])  # if user has enough cash
...
def sell():
    ...
    if request.method == "GET":
        stock_symbols = db.execute("SELECT symbol FROM transactions WHERE user_ID = :user_id;", user_id = session["user_id"],)
    ...
    if request.method == "POST":
        ...
        if stock_symbols == None:
            return apology("Sorry, you dont own that stock. choose again", 400)
...
def addcash():
    ...
    if request.method == "POST":
        ...
        # check user inputed cash
        if not request.form.get("cash"):
            return apology("please input amount of cash to add", 400)

        added_cash = request.form.get("cash")

        # add cash to cash table
        db.execute("UPDATE users SET cash = cash + :added_cash WHERE id = :user_id",
                    added_cash=added_cash, user_id=session["user_id"])
