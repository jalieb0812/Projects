...
def substrings(a, b, n):
    ...
    for sub_string in a:
        length = len(a)
        string = a[rangea1:rangea2]
        rangea1 = rangea1 + 1
        rangea2 = rangea2 + 1
        if rangea2 < len(a):
            substring_a_set.add(string)


    for sub_string in b:
        length = len(b)
        string = b[range1:range2]
        range1 = range1 + 1
        range2 = range2 + 1
        if range2 < len(a):
            substring_b_set.add(string)
   ...
   # y = substr(b, n)
    ...
    for substring in substring_a_set:
        if substring in substring_b_set:
            sub_string_list.append(substring)
